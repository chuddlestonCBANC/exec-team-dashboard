...
export function ExecutiveCard({ executive, isExpanded: initialExpanded = false, onSaveReport }: ExecutiveCardProps) {
  ...
  const metricsNeedingAttention = executive.ownedMetrics.filter((m) => {
    const status = getMetricStatus(m.currentValue, m.targetValue, { green: m.warningThreshold, yellow: m.criticalThreshold }, m.comparisonMode);
            ...
            <div className="p-5 border-t border-[var(--gray-100)]">
              ...
              <div className="space-y-2">
                {metricsNeedingAttention.map((metric) => {
                  const status = getMetricStatus(metric.currentValue, metric.targetValue, { green: metric.warningThreshold, yellow: metric.criticalThreshold }, metric.comparisonMode);
                  return (
                    <div
                      key={metric.id}
